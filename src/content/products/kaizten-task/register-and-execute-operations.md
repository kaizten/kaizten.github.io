...
![Kaizten Task architecture](/assets/products/kaizten-task/schema.png)
...
Required datasets for an operation must be uploaded to a folder and referenced as dataset objects in the operation request. The operation payload should include the dataset uuid or pre-signed URLs so the execution engine can access the data. Ensure the objects are readable by the service. The Kaizten-task API has 'METHOD /v1/datasets/folders'  to interact with folders and /v1/datasets/ to interact with the datasets.
